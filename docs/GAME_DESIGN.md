...
- **The ladder** (`data/dungeons.js`): 10 dungeons, each a 10-level band, each capped by a themed boss (Ashwing the Young, Malketh the Cold Lich, the Drowned King … Vurmalax the Elder Wyrm). All reuse the **seven shared room LAYOUTS** (shape / tiles / exits / blockers + an enemy composition of archetype figures) and re-skin them with a themed **roster** (each of the six figures renamed per theme), a **boss**, an accent colour, and a loot **power** — so we get ten distinct dungeons with no new floor art.
- **Enemies scale to Lv 100.** Each dungeon carries a `baseLevel`; foes spawn at **baseLevel + room depth**, the boss at the band's top. `models/units.scaleEnemy` grows the six archetype baselines by `base·(1 + rate·(level−1))` (rates in `BAL.ENEMY_SCALE`, roughly linear to track hero growth), so the tile **level badge** finally drives real stats — a Lv 95 wyrm is a genuine wall while the Emberdeep stays a Lv 1 tutorial.
...
- **Stay as long as you like — until the boss.** On the non-boss rooms, a cleared wave respawns after a short delay, so you can farm indefinitely; the fight never forces you onward. The **boss room is the finish line**: clearing it **wins the dungeon** (no respawn) — it marks the tier cleared, unlocks the next rung, grants the first-clear drop, and retires you to the Keep in victory. Each run now has a real end.
