...
/src/
  ...
  /ui/
    ...
    /screens/
      ...
      ForgeScreen.js  ShopScreen.js  BankScreen.js  TavernScreen.js  TempleScreen.js
...
**Phase 2 largely done** — endless-map loot loop: the **procedural `LootGenerator`** (prefix + material + gear type, drop-weighted per component in `data/items/*`, class-restricted, cosmetic grade from the rarest component, `lifesteal` proc wired end-to-end via `StatEngine.procVal` → `CombatSim`), drops-into-inventory on kill, `systems/Equipment.js`, and `ui/CharacterPanel.js`. The **Forge** is in too — `systems/ForgeSystem.js` (gem-fuelled `+N` upgrades, diminishing success + shatter risk, all odds in `balance.js`), surfaced via 🔨 in `CharacterPanel`. **8 gear slots** now (single source: `data/items/gearTypes.js` `SLOTS`, consumed by `makeHero` + the panel), an `Equipment.isUpgrade`/`itemScore` **▲ hint** on better drops, tap-a-slot bag **filtering**, and **silver** currency dropping from kills (with a small HUD). Still ahead in Phase 2: more procs (burn/chill need the status-effect pass) and a dedicated full-inventory screen with compare/sell.

**Phase 4 in progress** — a lightweight scene layer (`state.scene` = "town" | "dungeon"; the loop only ticks in "dungeon") plus a pre-game **onboarding flow** in `#flow`: `ui/Onboarding.js` (splash → guest login → class pick → roll stats → roll portrait → name). The main hero and companions are built dynamically (party is now `[main, ...hired]`, cap 4), so `mulberry32` was extracted to a pure `core/rng.js` and `models/units.js` gained `rollStats` / `makeHero(opts)` / `makeCompanion` + starter gear (`data/items/starter.js`, `data/names.js`). Screens: `ui/TownScreen.js` (Keep hub), `ui/ShopScreen.js` (buy/sell/reroll/gem), `ui/TavernScreen.js` (hire companions), `ui/TempleScreen.js` (resurrect the fallen), `systems/Economy.js` (pure pricing). Portraits are per-hero now (cached by portrait seed). **Economy of pals:** the player boots with `BAL.STARTING_SILVER` (enough for two Lv-1 hires + a reroll); recruits scale to the main hero's level via `models/units.js` `growTo(hero, level)` (applies `HERO_BASES[cls].growth` per level, refills HP), and both hire and resurrection fees scale with level (`BAL.TAVERN`/`BAL.TEMPLE`). **Death loop (pre-Bank):** a wipe returns the party to the Keep and **resets the battlefield** (`enterTown(true)` → `loadRoom`) so the next descent is a fresh delve; the main hero auto-revives free at partial HP, and fallen companions persist dead until raised at the Temple. `placeParty` is now idempotent — it adds only living members not already on the field, so a pal **hired or resurrected between delves joins on the next `enterDungeon`** (this fixes companions never appearing after boot). Fallen heroes render **greyed with a skull** everywhere they're shown — the dungeon party HUD, the Keep roster, and the Temple. The character panel shows each hero's **class**, and the Tavern shows your current party (tap to inspect stats) alongside the recruits. **Dungeon interaction:** the whole loop freezes (combat, slide/lunge interpolation, and FX all gate on it; `render` is handed `dt=0`) whenever the stats/gear panel is open — decoupled from the manual `phase` pause so opening a panel never touches the Fight/Pause button. The freeze is **derived from the overlay's real visibility** (`panelShown()` reads `#overlay.show`) rather than a separate boolean, so it can never get **stuck frozen** if the overlay is dismissed by any path — the single source of truth self-heals. The whole `loop` body is wrapped in a `try/catch` (logs once, keeps requesting frames) so a per-frame exception can never break the animation-frame chain into a hard freeze. Enemies spawn at **random open cells** (`randCell`, upper room band) each wave, and tapping the canvas plants a **rally flag** (`state.rally`) that idle heroes — those with no foe in `nearest()` — path toward via the existing `stepToward`. **Enemy tiles** fit the *whole* standing figure (`portOf` draws source `[24,44]→336²` from the 384² figure) instead of a humanoid head-crop, so low/wide creatures like rats are fully framed rather than cut off. Still ahead: a real `SceneManager` (bespoke wiring in `game.js` for now), the Bank, roster/bench management, and the world map.
...
- **`state.units` owns "who is on the battlefield right now."** Heroes in it are the *same object references* as in `party` (not copies); foes are the only members unique to it. It is rebuilt by `loadRoom` (`=[]` then `placeParty`+`spawnWave`) and pruned to living each tick. `placeParty` is **idempotent** — it adds only living party members not already present — so hiring/resurrecting a pal and descending re-adds them without disturbing the rest. Never store hero facts here that aren't also derivable from `party`.
- **Ephemeral UI state is derived, never mirrored.** The panel-open freeze reads `#overlay.show` (`panelShown()`), not a boolean; the pause label reads `state.phase`. When you need "is X showing?", read the DOM/state that already answers it rather than adding a parallel flag that can desync (the old `panelOpen`/`reviveAt` flags are what caused the freeze and revive bugs).
...
A deliberate next step (not yet done) is to make the battlefield hero list **fully derived** from `party.filter(alive)` rather than a manually-synced parallel array — that would remove the placement-sync class of bug entirely. It touches the hot loop, so it's a considered refactor, not a drive-by.
