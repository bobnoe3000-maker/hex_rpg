...
**Phase 4 started** — a lightweight scene layer (`state.scene` = "town" | "dungeon"; the loop only ticks in "dungeon") boots into the **Keep**: `ui/TownScreen.js` (hub — party, currency, services, Descend / 🏠), `ui/ShopScreen.js` (buy/sell/reroll/gem), and `systems/Economy.js` (pure pricing). Item-name rendering is unified in `ui/itemView.js`. Still ahead: a real `SceneManager` (this is bespoke wiring in `game.js` for now), the Bank, Barracks/roster, and the world map.
