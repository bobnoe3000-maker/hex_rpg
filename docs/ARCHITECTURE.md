...
**Phase 4 in progress** — a lightweight scene layer (`state.scene` = "town" | "dungeon"; the loop only ticks in "dungeon") plus a pre-game **onboarding flow** in `#flow`: `ui/Onboarding.js` (splash → guest login → class pick → roll stats → roll portrait → name). The main hero and companions are built dynamically (party is now `[main, ...hired]`, cap 4), so `mulberry32` was extracted to a pure `core/rng.js` and `models/units.js` gained `rollStats` / `makeHero(opts)` / `makeCompanion` + starter gear (`data/items/starter.js`, `data/names.js`). Screens: `ui/TownScreen.js` (Keep hub), `ui/ShopScreen.js` (buy/sell/reroll/gem), `ui/TavernScreen.js` (hire companions), `systems/Economy.js` (pure pricing). Portraits are per-hero now (cached by portrait seed). Still ahead: a real `SceneManager` (bespoke wiring in `game.js` for now), the Bank, roster/bench management, and the world map.
