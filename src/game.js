...
import { ROAM_LAYOUTS } from './data/roamLayouts.js';
...
/* Skin a validated geometry template (an existing hand-authored stack) into a 3-level roaming descent for
   another dungeon: reuse the room shapes / links / pack archetypes (like the classic shared LAYOUTS), but
   re-scale each pack's level into THIS dungeon's band and swap in its own mini-bosses + boss. The dungeon's
   theme (roster names, palette, tiles, boss) makes each read as its own place despite the shared layout. */
function genRoamStack(d, donor, donorBand){
  const b0=d.band[0], b1=d.band[1], dspan=Math.max(1, donorBand[1]-donorBand[0]);
  const mapLvl=L=>Math.max(b0, Math.min(b1, Math.round(b0 + (L-donorBand[0])/dspan*(b1-b0))));
  const minis=d.minis||[];
  return donor.map((lvl,i)=>{
    const isLast=i===donor.length-1;
    const mini = isLast ? undefined : (minis[i] || { fig:(lvl.miniboss&&lvl.miniboss.fig)||"wight", name:`Champion of ${d.sub}` });
    // names are barely surfaced (one log line uses names[0]); label the level by the dungeon's tagline
    const names=lvl.rooms.map((_,j)=> j===0 ? d.sub : (isLast ? "The Depths" : (j===lvl.rooms.length-1 ? mini.name : "The Depths")));
    return { cols:lvl.cols, rows:lvl.rows, rooms:lvl.rooms, links:lvl.links, entry:lvl.entry, names, miniboss:mini,
      packs: lvl.packs.map(pk=>({ room:pk.room, comp:pk.comp, treasure:pk.treasure, lvl:mapLvl(pk.lvl) })) };
  });
}
// EVERY dungeon is a roaming descent now. The two originals stay hand-authored; each of the rest is
// generated from its OWN geometry template (src/data/roamLayouts.js) — a distinct shape per dungeon —
// skinned with that dungeon's band, roster, mini-bosses and boss. Template pack levels are on a 1→10
// scale, so genRoamStack maps them into the dungeon's real band.
const ROAM_FLOORS={ emberdeep:EMBERDEEP_LEVELS, frostmere:FROSTMERE_LEVELS };
for(const d of DUNGEONS){
  if(ROAM_FLOORS[d.id] || !ROAM_LAYOUTS[d.id]) continue;
  ROAM_FLOORS[d.id]=genRoamStack(d, ROAM_LAYOUTS[d.id], [1,10]);
}
