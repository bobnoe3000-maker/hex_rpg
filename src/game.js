...
// EVERY dungeon is a roaming descent now. The two originals stay hand-authored; the rest are generated by
// skinning one of the two validated geometry templates (alternating by tier for layout variety).
const ROAM_FLOORS={ emberdeep:EMBERDEEP_LEVELS, frostmere:FROSTMERE_LEVELS };
for(const d of DUNGEONS){
  if(ROAM_FLOORS[d.id]) continue;
  const useEmber = d.tier%2===1;                                  // odd tiers → Emberdeep geometry, even → Frostmere
  ROAM_FLOORS[d.id]=genRoamStack(d, useEmber?EMBERDEEP_LEVELS:FROSTMERE_LEVELS, useEmber?[1,10]:[11,20]);
}
